...
let techName = $state(data.res.tech?.name || '')

$effect(() => {
  page = data.res.page;
  if (data.res.tech) {
    techName = data.res.tech.name;
  }
});
...
<main class={`project-main md:mx-4 mb-2 ${showSpinner ? 'blink-loading' : ''}`} id="app">
	...
	<div>
		...
		<h1 class="text-left text-1xl font-bold p-4 cursor-default -mt-4">Projects {techName ? 'in ' + techName: ''}: page {page} of {data.res.totalPages}</h1>
		{:else}
		<h1 class="text-left text-1xl font-bold p-4 cursor-default -mt-4">Projects {techName ? 'in ' + techName: ''}</h1>
		{/if}
