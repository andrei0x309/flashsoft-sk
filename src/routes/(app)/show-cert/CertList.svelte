					...
					{#if cert['cat']?.cat_name}
						<p class="text-right" style="margin-top: auto;">
							<small class="text-muted text-right">
								...
								><use
									href="/res/cat-icons.svg#cat-icon-{cert['cat']?.cat_icon_name}"
									xlink:href="/res/cat-icons.svg#cat-icon-{cert['cat']?.cat_icon_name}}"
								/></svg
							>
						</p>
					{/if}
...
{:else}
    ...
    <p class="mt-4">No Courses Found</p>
		<a href="/show-cert" class="backButton">Back</a>
	</div>
{/if}
