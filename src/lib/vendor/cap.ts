...
export function capMount() {
  let workerScript: BlobPart;
  ...
  class CapWidget extends HTMLElement {
    ...
    #solving = false;
    #eventHandlers: any;
    boundHandleProgress: any;
    boundHandleSolve: any;
    boundHandleError: any;
    boundHandleReset: any;
    ...
    constructor() {
      ...
      if (this.#eventHandlers) {
        this.#eventHandlers.forEach((handler: any, eventName: any) => {
      ...
      if (name.startsWith('on')) {
        ...
        if (value) {
          const handler = (event: any) => {
            const callback = this.getAttribute(name) as any;
            if (typeof window[callback] === 'function') {
              (window[callback] as any).call(this, event);
            }
          };
    ...
    async connectedCallback() {
      ...
      this.setWorkersCount(parseInt(workers as string) ? parseInt(workers as string, 10) : navigator.hardwareConcurrency || 8);
    ...
    async solveChallenges(challenge: string | any[]) {
  ...
  const workerFunct = function () {
    ...
    let initPromise, solve_pow_function: (arg0: any, arg1: any) => any;

    initPromise = import('https://cdn.jsdelivr.net/npm/@cap.js/wasm@0.0.3/browser/cap_wasm.min.js')
      .then((wasmModule) => {
        return wasmModule.default().then((instance: { exports: any }) => {
